...
import com.insightsurfface.myword.adapter.OnlyTextListAdapter;
...
public class ListDialog extends Dialog {
    ...
    private OnListDialogEventListener mOnListDialogEventListener;
...
//        lp.width = (int) (d.getWidth() * 1); // 閻庣妫勭�瑰磭鎷嬮崜褏鏋�
        ...
        if (null == onlyTextListAdapter) {
            ...
            optionsLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    if (null != mOnListDialogEventListener) {
                        if (null != list && list.length > 0) {
                            mOnListDialogEventListener.onItemClick(position);
                            mOnListDialogEventListener.onItemClick(list[position]);
                            if (null != codeList && codeList.length > 0) {
                                mOnListDialogEventListener.onItemClick(list[position], codeList[position]);
                            }
                            dismiss();
                        }
                    }
    ...
    public void setOnListDialogEventListener(OnListDialogEventListener onListDialogEventListener) {
        this.mOnListDialogEventListener = onListDialogEventListener;
    }
